  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      try {
        ...
        // Attempting to match your exact standard setup
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 dark:border-[#1a4a75] bg-gray-50 dark:bg-[#06243A]">
        <div>
          ...
          <p className="text-xs font-bold text-gray-500 dark:text-gray-400 mt-0.5">
            You have {notifications.filter(n => !n.read_at).length} unread messages
          </p>
        </div>
            ...
            >
              ...
              <div className="flex-1">
                <div className="flex justify-between items-start gap-2">
                  <h4 className={`font-bold text-sm ${notif.read_at ? "text-gray-800 dark:text-gray-200" : "text-[#09314F] dark:text-white"}`}>
                    {notif.data?.title || "Notification"}
                  </h4>
                  {!notif.read_at && <span className="w-2 h-2 rounded-full bg-[#E83831] flex-shrink-0 mt-1.5" />}
                </div>
                <p className="text-xs text-gray-600 dark:text-gray-400 font-medium mt-1 leading-relaxed">
                  {notif.data?.message || ""}
                </p>
      ...
      <div className="p-4 border-t border-gray-100 dark:border-[#1a4a75] bg-gray-50/50 dark:bg-[#06243A]/50 text-center">
        <button className="text-sm font-bold text-[#09314F] dark:text-blue-400 hover:underline">
          Mark all as read
        </button>
      </div>
    </div>
